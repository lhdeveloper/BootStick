...
1. Atualize `SCRIPT_VERSION` e `SCRIPT_BUILD` em `BootStick.command`
2. Commit e publique a tag:

```bash
git add BootStick.command
git commit -m "chore: release v1.4.1"
git tag v1.4.1
git push origin main --tags
```

Ou use o helper (lê a versão do script e cria a tag):

```bash
./scripts/release.sh          # cria tag vX.Y.Z
./scripts/release.sh --push   # cria tag e faz push
```

O workflow [`.github/workflows/release.yml`](.github/workflows/release.yml) gera o zip no GitHub Releases ao receber a tag (`v*` = `SCRIPT_VERSION`).
